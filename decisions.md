...
- **Domain registered.** `vibelanka.com` at Cloudflare with email forwarding configured for `partners@`, `team@`, `press@` → founder Gmail. Site mailto links use those addresses (PartnerSection card → `partners@`; CTA tinyPrint → `team@`; Footer Contact → `team@`; Footer Press → `press@`; Footer Become-featured → `partners@`).
