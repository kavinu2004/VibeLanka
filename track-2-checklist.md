...
- [~] **Build the login / auth flow.** Email / Google sign-in for v1. Account creation, sessions. _Backend plumbing built + pushed 2026-06-10 (Samithu, `f6e2c35`..`ffa1645`): `@supabase/ssr` browser+server clients, session middleware, `/auth/callback` code-exchange route, magic-link / Google / sign-out server actions, throwaway `/auth/test` page (Kavi owns real screens per D-010). Magic link = passwordless via Resend SMTP; Google code-path ships, activates on dashboard config. Spec `docs/superpowers/specs/2026-06-10-auth-flow-design.md`, plan `docs/superpowers/plans/2026-06-10-auth-flow.md`. Verified by `build`+`lint`+structural sweep. **NOT yet live-verified — live verification deferred by founder decision 2026-06-10:** SMTP/email delivery + Google OAuth put on hold. Code is complete and waiting; when email is wired later (Resend SMTP or built-in sender, redirect URLs, `.env.local` keys per spec §8), the live magic-link round-trip + `public.users` mirror check finishes the item._
